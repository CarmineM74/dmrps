...
describe Location do
  ...
  it "descrizione must be unique within each client's scope" do
    ...
    location_details = JSON.parse(FactoryGirl.build(:location).to_json(except: [:id,:created_at,:updated_at,:client_id]))
    location2_details = JSON.parse(FactoryGirl.build(:location,descrizione: location_details['descrizione']).to_json(except: [:id,:created_at,:updated_at,:client_id]))
